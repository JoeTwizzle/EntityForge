...
namespace EntityForge.Commands
{
    internal sealed class ArchetypeCommandBuffer : IDisposable
    {
        ...
        private readonly BitMask _knownEntityMask = new();
        private readonly BitMask _createdEntityMask = new();
        private readonly BitMask _moveIntoEntityMask = new();
        private readonly BitMask _destroyedEntityMask = new();
        //used for temporary operations
        private readonly BitMask _scratchTagAddMask = new();
        private readonly BitMask _scratchTagRemoveMask = new();
        private readonly OperationBuffer _operationBuffer = new();
        private readonly UnsafeSparseSet<UnsafeSparseSet> _virtualComponentStore = new(); //T.id, entityId
        private readonly UnsafeSparseSet<BitMask> _varyingComponentsMasks = new(); //T.id, entityId

        private int _createdEntities;
        private int _reservedEntities;
        private int _movedEntities;

        public ArchetypeCommandBuffer(Archetype archetype)
        {
        ...
        public int CreateMany(int firstId, int count)
        {
            lock (_lock)
            {
                _knownEntityMask.SetRange(firstId, count);
                _createdEntityMask.SetRange(firstId, count);
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        private int MoveInto(EntityId entity, ArchetypeCommandBuffer srcCmdBuf, ReadOnlySpan<OperationBuffer.Entry> opSpan)
        {
        ...
        private unsafe void StoreComponents(EntityId entity, int srcIndex, ArchetypeCommandBuffer srcCmdBuf)
        {
            var src = srcCmdBuf._archetype;
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        private void ExecuteTagChanges(ReadOnlySpan<OperationBuffer.Entry> opSpan, EntityId entity, TagBearer tag)
        {
            unchecked
            {
                ...
                {
                    long tagBitItem = (long)scratchBits[i];
                    while (tagBitItem != 0)
                    {
                ...
                for (int i = 0; i < len; i++)
                {
                    long tagBitItem = (long)scratchRemoveBits[i] & (long)tagBits[i];
                    while (tagBitItem != 0)
                    {
                        int bitIndex = i * (sizeof(ulong) * 8) + BitOperations.TrailingZeroCount(tagBitItem);
        ...
        internal unsafe void OnUnlock()
        {
            lock (_lock)
            {
                _archetype.Reserve(_reservedEntities);
                _reservedEntities = 0;
